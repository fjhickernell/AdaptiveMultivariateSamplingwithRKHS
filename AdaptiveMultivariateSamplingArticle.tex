...
\begin{example}
	...
	K(\bt,\bx) = (1 + \theta \norm{\bt-\bx}) \exp(-\theta\norm{\bt-\bx}),
	\end{equation}
	where $\norm{\cdot}$ denotes the Euclidean norm, and $\theta = 1$.
...
Let $\{\calf_{\btheta} : \btheta \in \Theta\}$ be a family of RKHSs parameterized by $\btheta$.  An example is the Mat\'ern family defined in terms of a scalar parameter in \eqref{eq:Matern}.  More diverse families are possible.  From this section forward, we highlight the dependence of the reproducing kernel, the function approximation, and related quantities on $\btheta$ by a subscript.

Let $\vBall$ denote the volume of a unit ball in $n$ dimensions, and let $\vEll(\btheta,\mX,\by)$ denote the volume of the ellipsoid in $\reals^n$ consisting of all possible function data whose minimum-norm interpolants have an $\calf_{\btheta}$-norm no greater than that of the interpolant from the observed data:
\begin{align*}
...
The volume of the ellipsoid, $\vEll(\btheta,\mX,\by)$, can be considered a measure of how large the observed function data is with respect to the Hilbert space $\calf_{\btheta}$.  This measure is independent of the vertical scale of the reproducing kernel.  They do not change if the reproducing kernel $K_\btheta$ is replaced by the reproducing kernel $cK_{\btheta}$ for any positive constant $c$.  

Figure \ref{fig:ellipPlot} displays the Mat\'ern kernel in \eqref{eq:Matern} for three different values of $\btheta$ and the corresponding  ellipsoids $\{ \bz \in \reals^n : \bz^T \mK_\btheta^{-1} \bz \le \by^T \mK_\btheta^{-1} \by  \bigr \}$  defined for a two-sample design.  Of these, $\btheta = 2$ has the smallest volume (area).

The reproducing kernel describes how close to each other two values of a function may be:
...
\end{equation}
The Mat\'ern kernel with small $\theta$ is relatively flat, which means that function values do not change much with location.  This explains why the ellipsoid for $\theta = 0.5$ hugs the line $z_1 = z_2$.  For large $\theta$ the Mat\'ern kernel is peaked, which explains why the ellipsoid for $\theta = 8$ is more spherical in shape.
...
We argue that the most suitable kernel is the one that minimizes the volume of the ellipsoid.   That is, our choice of $\btheta$ is
...
\end{equation}
A small volume of the ellipsoid means that this data is considered small in the RKHS $\calf_{\btheta}$-norm as what is observed. 
