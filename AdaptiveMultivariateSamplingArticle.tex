...
Although we explained in the introduction that we are primarily concerned with the computational cost of obtaining function data, we will count the cost of our algorithms.  There are a number of matrix operations that require $\Order(n^3)$ operations.  Explicitly showing the $n$ dependence, 
\begin{gather*}
\by_1, \ldots, \by_n, \quad K(\bx,\mX_1), \ldots, K(\bx,\mX_n)  \quad \text{can be be computed iteratively in $\Order(n)$ operations} , \\
\mK_1, \ldots, \mK_n  \quad \text{can be be computed iteratively in $\Order(n^2)$ operations}
\end{gather*}
Let $\mK  = \mL \mL^T$ be the Cholesky decomposition of the Gram matrix $\mK$, where $\mL$ is a lower triangular matrix.  Then $\mK^{-1} = \mU \mU^T$, where $\mU = \mL^{-T}$ is an upper triangular matrix.  
