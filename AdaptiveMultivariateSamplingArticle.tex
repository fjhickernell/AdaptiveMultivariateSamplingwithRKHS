...
\begin{subequations} \label{eq:RKHSAPP_P}
	...
	\end{align}
\end{subequations}
For any design $\mX$ and function data $\by$, let $\calg(\mX,\by) = \{ g = g_0 + g_{\btheta} : g_0 \in \calf_0, \ g_{\btheta} \in \calf_{\btheta}, \ g(\mX)  = \by\}$.  Then $\APP_\btheta(\mX,\by) $ is the element of $\calg(\mX,\by)$ with minimum $\norm[\calf_\btheta]{g_{\btheta}}$.
