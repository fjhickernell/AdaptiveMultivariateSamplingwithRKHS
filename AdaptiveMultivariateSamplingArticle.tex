...
\title{Adaptive Multivariate Sampling via Minimum Norm Interpolants for Cones of Functions}
\author{}
\date{\today}
...
\begin{example}
	...
	\input{ProgramsImages/Example1Table.txt}
...
Algorithm \ref{alg:adaptsample} uses fewer data sites that Algorithm \ref{alg:basicadapt} while still satisfying the overall error bound \eqref{eq:DataErrBdB} as well also satisfying the pointwise error bound \eqref{eq:DataErrBdA}.
		...
		\input{ProgramsImages/Example2Table.txt}
...
\begin{example}
	...
	\input{ProgramsImages/Example3Table.txt}
	
	
	\begin{figure}[H]
		\centering
		\includegraphics[height = 5.5cm]{ProgramsImages/SimpleFunAlg3.eps}
...
To illustrate it is still a space-filling design,  we consider the following univariate function, which has a mean value near zero
 ...
 \exp(-6x)\sin(8x+0.1) - 0.1,  \qquad x \in [0,1],
	\end{equation}
	which is plotted in Figure \ref{fig:ex4}.  We apply Algorithm \ref{alg:adapttheta} to this example using the sequence of data sites described in \eqref{eq:FixedVDCDes} with $n_0=3$ and for a sequence of error tolerances.  We choose the generalization of the Mat\'ern kernel in \eqref{eq:MaternOne},  $A_\infty = B_0 = 0.2$, and $\mT = (0, 0.001, \ldots, 1)^T$.  This table summarizes our results.
	
	
	\input{ProgramsImages/Example4Table.txt}
	
	
	\begin{figure}[H]
		\centering
		\includegraphics[height = 5.5cm]{ProgramsImages/UniFunPlot.eps} \qquad
		\includegraphics[height = 5.5cm]{ProgramsImages/UniFunAlg3.eps}
		\caption{ \label{fig:ex4}}
	\end{figure}

However, unlike Example  \ref{ex:adpdatachooseth}, the overall and pointwise error bounds do not hold for small values of $n$ in this example; pointwise error bounds do not hold even for larger value of $n$.
...
\YuhanNote{Should we choose the same range
for $\bb$ \ as $\ba$}
