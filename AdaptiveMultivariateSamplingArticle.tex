...
To recover all pure trends exactly, we modify our approximation in \eqref{eq:RKHSAPP} to take the form 
...
The function, $f$, to be approximated is the sum of two parts: $f_t = (\bgamma + \bdelta)^T \bp \in \calf_t$,  $f_{\btheta\parallel} = (\bc + \bDelta)^T K_\btheta(\mX,\cdot)$ and $f_{\btheta\perp} \in \{g \in \calf_\btheta : g(\mX) = \bzero\}$.  Due to the interpolation condition, $\mK_\btheta \bDelta  + \mP \bdelta = \bzero$, so $\bDelta = - \mK_\btheta^{-1} \mP \bdelta$.
...
\\
\nonumber
& \le \sqrt{\bp^T(\bx)\bp(\bx) - 2 \bp^T(\bx)\mP^T \mK_\btheta^{-1}K_\btheta(\mX,\bx)  
	+  K_\btheta(\bx,\mX)  \mK_\btheta^{-1} \mP \mP^T \mK_\btheta^{-1}K_\btheta(\mX,\bx)  } \norm{\bdelta} \\
\nonumber
 & \qquad \qquad + \sqrt{K_\btheta(\bx,\bx) - K_\btheta(\bx,\mX) \mM_\btheta K_\btheta(\mX,\bx)} \norm[\calf_\btheta]{f_{\btheta\perp}(\bx)} 
\\
\nonumber
