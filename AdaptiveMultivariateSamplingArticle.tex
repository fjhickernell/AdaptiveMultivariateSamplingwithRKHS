...
\end{itemize}
Our theory develops progressively by adding additional forms of adaption for functions lying purely in a RKHS.  In Section \ref{sec:poly} and Algorithm \ref{alg:poly} we generalize to the setting where a low-dimensional space of possible trends, e.g., polynomials of fixed degree, are exactly reproduced.
...
\end{align}
\end{subequations}
Here, $\errK(\mX,\bx)$ represents the norm of the error functional for approximating $f(\bx)$ by $\APP(\mX,\by)(\bx)$.  It is also the represents the largest possible value of $\abs{g(\bx)}$ for any $g$ of unit norm in the subspace $\calf_{\mX \perp}$ of functions that vanish at the data sites.
...
\label{ex:adpsamplesize}
	...
	We have \[f: x \mapsto 2 (\exp(-10(x-1/2)^2)- 0.6), \qquad \varepsilon = 0.02.\]
	Here, we use the van der Corput sequence mentioned above with the initial sample size $n_0=1$.
	Let's choose $A_\infty = 0.1$ and $B_0 =0.2$ to implement Algorithm
	\ref{alg:basicadapt}.
	We finally get $n=14$ to satisfy the error tolerance with true error 0.0157.
\end{example}
...
\label{ex:adpdataselect}
	Here we use the same kernel and error tolerance as in Example \ref{ex:adpsamplesize}.
	
	\[K(\bt,\bx) = (1 +  \norm{\bt-\bx}) \exp(-\norm{\bt-\bx}),\qquad \varepsilon = 0.02.\]
	
We also use the van der Corput sequence mentioned above with the initial sample data site $\bx_1=1/3$.
	Let's choose $A_\infty = 0.1$ and $B_0 =0.2$ to implement Algorithm
	\ref{alg:adaptsample}.
	
	We finally get $n=7$ to satisfy the error tolerance with true error 0.0109. The number of points is half as in Example \ref{ex:adpsamplesize},
	as we can use the acquisition function to find the next point.
\end{example}
...
However, we will use the generalization of the Mat\'ern kernel  in \eqref{eq:MaternTheta}.
We still use the van der Corput sequence mentioned above with the initial sample data site $\bx_1=1/3$.
	Let's choose $A_\infty = 0.1$ and $B_0 =0.2$ to implement Algorithm
	\ref{alg:adaptsample}.
