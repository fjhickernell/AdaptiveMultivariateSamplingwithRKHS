...
\newcommand{\aneye}{\text{\fontsize{4}{5}\selectfont \faEye}}
\newcommand{\noeye}{\text{\fontsize{4}{5}\selectfont \faEyeSlash}}
...
\begin{itemize}
    ...
    \item Adaptive choice of the Hilbert space, $\calf$, for which the input function, $f$, is typical  (Section \ref{sec:adaptF} and Algorithm \ref{alg:adapttheta}).
\end{itemize}
...
\begin{itemize}
	\item The candidate cone (Section \ref{sec:basicIdea}) and
	\item A geometric interpretation of the data-based inference of a suitable RKHS (Section  \ref{sec:adaptTheta}).
\end{itemize}

Any algorithm that claims to satisfy \eqref{eq:errorcrit} cannot possibly do so for $\cc$ corresponding to an infinite dimensional vector space of functions, $\calf$.  If such an algorithm existed, then $\ALG(0,\varepsilon)$ would terminate after evaluating the zero function at some data sites $\bx_1, \ldots, \bx_n$ for some positive integer $n$.  Since $\cc = \calf$ is infinite dimensional, there exists some $g \in \cc$, for which $g(\bx_1) = \cdots = g(\bx_n) = 0$ but for which $\norm[\infty]{g} > 2 \varepsilon$.  That is, $g$ looks like  $0$ to the algorithm so $\ALG(g,\varepsilon) = \ALG(0,\varepsilon)$, however, $g$ is not close enough to $0$.   Since $\ALG(0,\varepsilon)$ cannot be within distance $\varepsilon$ of both the $0$ and $g$, error criterion \eqref{eq:errorcrit} cannot be satisfied.  Thus, no matter how nicley we choose our \emph{infinite} dimensional vector space of possible function to approximate, $\calf$, the set for which our adaptive algorithm succeeds, $\calc$ must be a proper subset of $\calf$.
...
\end{align}
\end{subequations}
This approximation, $\APP(\mX,\by)$, lies in $\calf_{\mX} := \{\bc^T K(\mX,\cdot) : \bc \in \reals^n \}$, an the $n$-dimensional subspace of $\calf$.  The pointwise approximation error, $f - \APP(\mX,\by)$, lies in $\calf_{\mX \perp} := \{g \in \calf : g(\mX) = \bzero\}$, the orthogonal infinite dimensional subspace of $\calf$ that is orthogonal to $\calf_{\mX}$.
...
The data-based approximation $\APP(\mX,\by)$ in \eqref{eq:RKHSAPP} is an ingredient in our adaptive algorithm, but error bound \eqref{eq:RKHSErrBd} contains the unknown factor $\norm[\calf]{f - \APP(\mX,\by)}$, which cannot be computed from function data.  What we can compute from data is  $\bignorm[\calf]{\APP(f)} = \sqrt{\by^T \mK^{-1} \by}$  via vector-matrix operations.  Moreover, since the error is orthogonal to the approximation,  the Pythagorean Theorem implies that $\norm[\calf]{f}^2  = \bignorm[\calf]{\APP(f)}^2 + \bignorm[\calf]{f - \APP(f)}^2$.  To establish a data-driven error bound, we define this candidate cone as follows:
...
This algorithm is deficient.  The choice of the design does not yet depend  on the function data.  There is no assurance that the RKHS specified by the choice of $K$ fits $f$.  There is also the matter of how to choose $A(\mX)$.  These matters are addressed in the following (sub-)sections.
...
%	\input{ProgramsImages/AdaptAlgo1_Out_sinFun_GaussKernel_adapt_th_EmpBayesAx_theta_0.txt}
	\begin{figure}[H]
		\centering
...
%	\input{ProgramsImages/AdaptAlgo2_Out_sinFun_GaussKernel_adapt_th_EmpBayesAx_theta_0.txt}
...
%	\input{ProgramsImages/AdaptAlgo3_Out_sinFun_GaussKernel_adapt_th_EmpBayesAx_theta_0.txt}
...
\begin{example}
	...
	\input{ProgramsImages/AdaptAlgo1_Out_HigdonFun_GaussKernel_adapt_th_EmpBayesAx_theta_1.txt}\\
		\input{ProgramsImages/AdaptAlgo1_Out_HigdonFun_GaussKernel_adapt_th_EmpBayesAx_theta_2.txt}\\
	...
	\input{ProgramsImages/AdaptAlgo2_Out_HigdonFun_GaussKernel_adapt_th_EmpBayesAx_theta_1.txt}\\
	\input{ProgramsImages/AdaptAlgo2_Out_HigdonFun_GaussKernel_adapt_th_EmpBayesAx_theta_2.txt}\\
	
But for Algorithm \ref{alg:adapttheta}, it can reach all the error tolerance $\varepsilon = 1E-5$ with 25 points.
	
	\input{ProgramsImages/AdaptAlgo3_Out_HigdonFun_GaussKernel_adapt_th_EmpBayesAx_theta_1.txt}
		\input{ProgramsImages/AdaptAlgo3_Out_HigdonFun_GaussKernel_adapt_th_EmpBayesAx_theta_2.txt}
\end{example}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
For a smooth kernel such as the squared exponential kernel one defined in \eqref{eq:GaussTheta}, the Gram matrix, $\mK$, may not be of full rank numerically.  This requires some adjustment to the formula for the approximation, $\APP(\mX,\by)$, and to our error estimates.

Figure \ref{fig:rankKPlot} displays the numerical rank of the Gram matrix as a function of $n$ for various parameter values of the squared exponential kernel.  For infinite precision, the numerical rank would be the same as $n$, which is the case for small $n$.  One can decompose the Gram matrix as
\begin{equation}
	\mK = \mV \mLambda \mV^T = \mK_\aneye + \mK_\noeye, \qquad \mK_\aneye = \mV_{\aneye}\mLambda_\aneye \mV^T_\aneye, \quad 
	\mK_\noeye = \mV_{\noeye}\mLambda_\noeye \mV^T_\noeye,
\end{equation}
where $\mK$ represents the part of $\mK$ that is visible in machine precision, and $\mK_\noeye$ represents the part of $\mK$ that is invisible.  The matrices $\mV$, $\mV_\aneye$, and $\mV_\noeye$ represent the eigenvectors of $\mK$, $\mK_\aneye$, and $\mK_\noeye$, respectively, and $\mLambda$, $\mLambda_\aneye$, and $\mLambda_\noeye$ are diagonal matrices containing the eigenvalues of $\mK$, $\mK_\aneye$, and $\mK_\noeye$, respectively.  The numerical rank of $\mK$ corresponds to the rank of $\mK_\aneye$, which equals the number of rows or columns of $\mLambda_\aneye$. 

\begin{figure}
	\includegraphics[height = 5cm]{ProgramsImages/rankKernel.eps}
	\caption{The numerical rank of $\mK$ as a function of $n$ corresponding to the squared exponential kernel \eqref{eq:GaussTheta} for $d = 1$ and various parameters $\theta$.  For the flatter kernels (smaller $\theta$), the numerical rank falls below $n$ for smaller $n$.}
	\label{fig:rankKplot}
\end{figure}
...
To illustrate it is still a space-filling design,  we consider the following univariate function, which has a mean value near zero
 ...
 \exp(-6x)\sin(8x+0.1) - 0.1,  \qquad x \in [0,1],
	...
	\begin{figure}[H]
		...
		\includegraphics[height = 4.5cm]{ProgramsImages/Alg3_LeftPeakFun_Gauss_adapt_th_EmpBayesAx_theta_1.eps} \qquad
	...
	\end{figure}
	We apply Algorithm \ref{alg:adapttheta} to this example using the sequence of data sites described in \eqref{eq:FixedVDCDes} with $n_0=3$. We choose the generalization of the Gaussian kernel in \eqref{eq:GaussOne},  $A_\infty =1, B_0 = 0.05$, and $\mT = (0, 0.001, \ldots, 1)^T$. 
	


		
	\input{ProgramsImages/Alg3_Out_LeftPeakFun_Gauss_adapt_th_EmpBayesAx_theta_1.txt}
...
In the example above the function oscillates more on the left than the right, but \eqref{eq:diff_f_Gauss} for the Gaussian kernel says that the maximum difference between function values at two locations is only depends on the distance between the locations.  The same holds for all $K(\bt ,\bx)$ that are  functions of $\bt - \bx$.  Even for more general kernels, we want to be able to infer their location dependence from the data, not have it baked in.

For any reproducing kernel, $K(\bt ,\bx)$, it follows that $w(\bt)w(\bx)K(\bt ,\bx)$ is also a reproducing kernel regardless of the choice of $w$.  We can generalize the Gaussian kernel in \eqref{eq:GaussTheta} further as follows and apply \eqref{eq:diff_f}:
\begin{align} \label{eq:GaussAB}
K_\btheta(\bt,\bx) &= \exp(\bb^T(\bt + \bx))(1 +  \norm{\ba \odot (\bt-\bx)}) \exp(-\norm{\ba \odot (\bt-\bx)}),  \quad \btheta=(\ba, \bb), \\
\nonumber
%\label{eq:diff_f_GaussAB}
...
\label{ex:compfun2}
	\begin{figure}[H]
		\centering
		\includegraphics[height = 4.5cm]{ProgramsImages/Alg3_LeftPeakFun_SpatialGauss_adapt_th_EmpBayesAx_theta_1.eps} 
		...
		%\includegraphics[height = 5.5cm]{ProgramsImages/Alg3_LeftPeakFun_SpatialGauss2.eps} 
		\caption{ \label{fig:ex5}}
	\end{figure}

		
	\input{ProgramsImages/Alg3_Out_LeftPeakFun_SpatialGauss_adapt_th_EmpBayesAx_theta_1.txt}
