...
According to \ref{eq:RKHScone}, the factor  $A$ reflects how much larger one expects $\bignorm[\calf]{f  - \APP(\mX,\by)}$ to be than $\bignorm[\calf]{\APP(\mX,\by)}$ is.  This should depend on the quality of the design and the number of data sites.  One measure of the quality of the design is 
\begin{equation*}
