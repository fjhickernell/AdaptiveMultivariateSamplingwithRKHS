...
approximate $\norm[\infty]{K(\cdot,\cdot) - K(\cdot,\mX) \mK^{-1} K(\mX,\cdot)}$, which appears in via the data-based error bound in \eqref{eq:DataErrBdB} as well as the definition of $B(\mX)$ in \eqref{eq:BX}.  Optimization is also required below in \eqref{label} for choosing data sites and in \eqref{} below.  Identifying the best numerical optimization algorithms is beyond the scope of this article and are definitely routines for performing these optimizations here.
