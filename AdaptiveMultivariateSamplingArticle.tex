...
approximate the sup-norms that appear in via the data-based error bound in \eqref{eq:DataErrBdB} and the definition of $B(\mX)$ in \eqref{eq:BX}.  Finding the location of optima is required below in \eqref{label} for adaptively choosing the next data site and for adaptively choosing the correct kernel in \eqref{} in Section \ref{label}.  

Identifying the best numerical optimization algorithms for performing these optimizations is beyond the scope of this article and are definitely routines  here.
